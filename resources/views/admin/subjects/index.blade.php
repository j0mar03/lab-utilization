        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                    Maintain curriculum course codes, titles, and department mappings for lab utilization and schedules.
                </p>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                <div class="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm">
                    <p class="text-xs text-gray-500 dark:text-gray-400 font-medium">Total Subjects</p>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
                <form method="GET" action="{{ route('admin.subjects.index') }}" class="flex flex-wrap gap-3 items-end">
                    <div class="flex-1 min-w-[200px]">
                        <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1">Search Code / Title</label>
                        <input type="text" name="search" value="{{ request('search') }}"
                               placeholder="e.g. CPET, INTE, Circuits, Database..."
                        ...
                        <select name="department" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm px-3 py-2 shadow-sm">
                            ...
                            @foreach ($departments as $dept)
                                <option value="{{ $dept }}" {{ request('department') === $dept ? 'selected' : '' }}>
                                    {{ \App\Models\Subject::DEPARTMENTS[$dept] ?? $dept }} — {{ Str::limit($dept, 25) }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="w-44">
                        ...
                        <select name="year_level" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm px-3 py-2 shadow-sm">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if (request()->hasAny(['search', 'department', 'year_level', 'status']))
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                        <thead class="bg-gray-50 dark:bg-gray-700/50 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                            <tr>
                                ...
                                <th class="px-5 py-3 text-left">Course Code & Name</th>
                            ...
                            @forelse ($subjects as $subject)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                                    ...
                                    <td class="px-5 py-3.5">
                                        <div class="flex items-center gap-2">
                                            <span class="inline-flex items-center px-2 py-0.5 rounded font-mono font-bold text-xs bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-600">
                                                {{ $subject->code }}
                                            </span>
                                    ...
                                    <td class="px-5 py-3.5 text-right whitespace-nowrap space-x-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.subjects.destroy', $subject) }}"
                                              class="inline"
                                              onsubmit="return confirm('Are you sure you want to remove {{ addslashes($subject->code) }}?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
